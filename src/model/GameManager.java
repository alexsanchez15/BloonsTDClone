...
public class GameManager {
	...
	public static int blockSize = resX / 16; // or resY / 9, 100x100 pixel gridsize for 1600x900
							...
							// balloon sizes ill just hold locally lol i dont hate the ovals
							// (for now i suppose)
	...
	Image marginImage = new Image(GameManager.class.getResource("/sprites/layout/marginscreen.png").toExternalForm());
	...
	public void draw(GraphicsContext gc) {
		...
		// probably not the best way to do this, but i like how clicked items are drawn
		// over all other shop tiles
		for (ShopTile t : shopTiles) {
			t.drawClicked(gc);
		}
	...
	public void handleMouseMovement(int x, int y) {
		...
		if (shopTiles != null) {
			for (ShopTile t : shopTiles) {
				...
				if ((x >= t.getTrueX() && x <= t.getTrueX() + blockSize)
						&& (y >= t.getTrueY() && y <= t.getTrueY() + blockSize)) {
					t.setHovered(true);
	...
	public void handleMouseClick(int x, int y) {
		// 3 cases (i think) where this is needed, buying from the shop, clicking on
		// towers, placing towers being bought.

		// handle mouse click for towers that are placed

		if (towers != null) {
			for (Tower t : towers) {
				if ((x >= t.getX() - t.getRadius() && x <= t.getX() + t.getRadius())
						&& (y >= t.getY() - t.getRadius() && y <= t.getY() + t.getRadius())) {
		...
		if (shopTiles != null) {
			...
			for (ShopTile t : shopTiles) {
				...
				else if ((x >= t.getTrueX() && x <= t.getTrueX() + blockSize)
						&& (y >= t.getTrueY() && y <= t.getTrueY() + blockSize)
						&& t.getThisTower().getCost() <= playInfo.getMoney()) {
					t.setMouseCoords(x, y);
					t.isClicked(true);
					// pay for the tower
